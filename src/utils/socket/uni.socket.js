...
export default class Socket {
  constructor(option = {}) {
    ...
    this._heart_data = option.heartData || {}
   ...
   */
  async getState() {
    return this.begin_reconnection
      ? 2
      : this._connectioned
      ? 1
      : this.isError
  ...
  async emit(event, data = {}) {
    if (
      await this.getType(event) === "[object Object]" &&
      await this.getType(event) === "[object String]"
    ) {
      let e = data;
      data = event;
      event = e;
  ...
  async error(err) {
    ...
    if (this.on_register["error"] !== undefined) {
      await this.invokeHandlerFunctionOnRegister("error", err);
    }
  }
  ...
  async reconnectionError(err) {
    ...
    if (this.on_register["reconnectionerror"] !== undefined) {
      await this.invokeHandlerFunctionOnRegister("reconnectionerror", err);
    }
  }
  ...
  async connectioned() {
    ...
    if (this.on_register["connectioned"] !== undefined) {
      await this.invokeHandlerFunctionOnRegister("connectioned");
    }
    // 给服务器发送心跳
    await this.beginSendHeartBeat();
  }
  ...
  async beginSendHeartBeat() {
    this._heart_rate_interval = setInterval((res) => {
      this.emit(this._heart_rate_type, this._heart_data);
  ...
  async emitMessageToTargetEventByName(name, data) {
    await this.invokeHandlerFunctionOnRegister(name, data);
  }
  ...
  async emitToClientNotNameEvents(msg) {
    await this.invokeHandlerFunctionOnRegister("**", msg);
  }
  ...
  async emitToClientAllEvent(data) {
    await this.invokeHandlerFunctionOnRegister("*", data);
  }
   ...
   */
  async invokeHandlerFunctionOnRegister(register, data) {
